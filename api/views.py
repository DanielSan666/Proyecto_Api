...
def participantes(request):
    if request.user.is_authenticated:
        ...
        print(usuario.disciplina)
        try:
            partis = Parti.objects.filter(disciplina=usuario.disciplina)
...
def importar_registros(request):
    if request.method == "POST":
        ...
        for fila in hoja.iter_rows(min_row=2, values_only=True):
            ...
            disciplina = fila[6]
            rama = fila[7]
            fecha_ingreso_str = fila[
                8
            ]  # Obtener el valor de fecha como una cadena desde el archivo Excel
        ...
        return redirect(
            "index"
        )  # Redirigir a la página de entrenadores después de importar los registros
